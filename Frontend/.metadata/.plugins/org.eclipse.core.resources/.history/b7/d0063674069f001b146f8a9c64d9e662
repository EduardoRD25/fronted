...
<body>
	...
	<div class="container">
		<h3 align="center">Listado de Clientes</h3>
					...
					id="validateBtnw2" 
					onclick="mostrarVentana()">Nuevo Cliente</button><br>&nbsp;<br>		
			<div id="divEmpleado">
 ...
 <div class="modal fade" id="idModalRegistra" >
			<div class="modal-dialog" style="width: 50%">
				...
				<div class="modal-header" style="padding: 5px 20px">
					<h4 id="idMensaje">Registro de Cliente</h4>
				</div>
...
<script type="text/javascript">
	...
	function tablaCliente(){
		...
		$.getJSON("listaClientes",{}, function (response){
			$.each(response.dataClientes, function(index, item){
				...
				$("#id_table_cliente").append("<tr><td>"+item.cod_cli+"</td>"+
												  "<td>"+item.nom_cli+"</td>"+
												  "<td>"+item.ape_cli+"</td>"+
												  "<td>"+item.dni_cli+"</td>"+
												  "<td>"+item.sexo_cli+"</td>"+
			...
			$("#id_table_cliente").DataTable().draw();
	...
	function eliminar(cod){
		...
		swal({
			  title: "Seguro de eliminar el siguiente Cliente con Código : "+cod,
			  text: "",
			  icon: "warning",
			  buttons: true,
			  dangerMode: true,
	...
	function editar(cod,nom,ape,dni,sex,dis){
		...
		$("#idSexo").val(sex);
		$("#idDistrito").val(dis);
		$("#idMensaje").text("Actualizar Cliente");
